...
console.log('🔧 解析到的 install 参数（敏感信息已脱敏）');
...
console.log('🔧 合并后的 fileConfig（敏感信息已脱敏）');
...
const DISABLE_ARGO = getConfig('DISABLE_ARGO', 'DISABLE_ARGO', 'true');      // 设置为 true 时禁用argo,false开启
...
console.log(' GH_TOKEN:', GH_TOKEN ? '[redacted]' : '[unset]');
...
// Lightweight domain validation (avoid treating bare IPs as domains)
function isValidDomainName(d) {
  if (typeof d !== 'string') return false;
  // Must contain at least one dot and include alphabetic characters
  // Reject pure IP-like patterns (e.g., 1.2.3.4)
  const hasDot = /\./.test(d);
  const hasAlpha = /[A-Za-z]/.test(d);
  const validChars = /^[A-Za-z0-9.-]+$/.test(d);
  const endsWithTld = /\.[A-Za-z]{2,}$/.test(d);
  return hasDot && hasAlpha && validChars && endsWithTld;
}
...
async function generateLinks(argoDomain) {
  ...
  // Fallback for argo domain if none provided by ARGO_DOMAIN/ENV
  // Determine final argo domain for TLS SNI and host header
  const argoDomainFinal = (argoDomain && isValidDomainName(argoDomain))
    ? argoDomain
    : (PAPER_DOMAIN && isValidDomainName(PAPER_DOMAIN) ? PAPER_DOMAIN : SERVER_IP);
  // Debug/verification aid: show resolution path
  console.log(`🔎 Argo域决策: argoDomain=${argoDomain || 'null'}, PAPER_DOMAIN=${PAPER_DOMAIN}, SERVER_IP=${SERVER_IP}, argoDomainFinal=${argoDomainFinal}`);
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      if ((DISABLE_ARGO !== 'true' && DISABLE_ARGO !== true) && argoDomain) {
        ...
        console.log(`🧭 Argo IP 选择: PAPER_ARGO_IP='${PAPER_ARGO_IP}' CFIP='${CFIP}' -> argoIP='${argoIP}'`);
        let vmessNode;
...
// Robust startup: guard against invalid PORT and handle startup errors with port fallback
let listenPort = Number.isFinite(PORT) ? PORT : 3000;
let server;
function tryListen(port) {
  server = app.listen(port, () => console.log(`server is running on port:${port}!`));
  server.on('error', (err) => {
    if (err && err.code === 'EADDRINUSE' && port < 65535) {
      console.warn(`Port ${port} in use, trying port ${port + 1}...`);
      tryListen(port + 1);
    } else {
      console.error(`Failed to start OpenCode web server on port ${port}:`, err);
      process.exit(1);
    }
  });
}
